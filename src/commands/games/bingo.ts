import { Command, Arguments, Permission, Server, validatePermissions, notNull, discordErrorHandler, Emojis, defaultErrorHandler, Time } from "../../defs";
...
function processGame() {
	...
	game.message.edit(`Números jogados (use \`!!bingo\` quando encontrar todos os seus números):\n\`\`\`${game.played.join(' ')}\`\`\``)
		.then(() => {
			setTimeout(processGame, Time.second * 7);
		})
...
export default <Command>{
	async run(msg: Message, args: Arguments, raw: string[]) {
		if (args.length > 1) { // iniciando um bingo
			...
			if (!validatePermissions(notNull(msg.member), <TextChannel>msg.channel, Permission.MOD))
				return msg.reply("você não tem permissão para iniciar um bingo").catch(discordErrorHandler);

			if (game !== undefined)
				return msg.reply("já tem um bingo rolando (se você está tantando chutar, use esse comando sem argumentos)").catch(discordErrorHandler);

			if (args[1].kind !== "NUMBER")
				return msg.reply("o tamanho da tabela tem que ser um número").catch(discordErrorHandler);

			const tableSize = args[1].value;

			if (tableSize > 7 || tableSize === 0)
				return msg.reply(`o tamanho da tabela tem que estar em [1, 6]`).catch(discordErrorHandler);

			msg.channel.send(`Bingo!\nPressione na reação ${Emojis.circle} para participar. O host irá apertar na reação ${Emojis.yes} para iniciar!`)
				.then(async message => {
					message.react(Emojis.yes).catch(discordErrorHandler);
					message.react(Emojis.circle).catch(discordErrorHandler);
					...
					message.awaitReactions(filter, { maxUsers: 1 }).then(async collected => {
						...
						const reaction = await message.reactions.cache.get(Emojis.circle)?.fetch();
						if (!reaction)
							return;
						...
						if (Object.keys(game.players).length < 2) {
							msg.reply("precisa ter no mínimo 2 jogadores para iniciar o bingo").catch(discordErrorHandler);
							game = undefined;
							return;
						}

						setTimeout(processGame, Time.second * 10);
					}).catch(defaultErrorHandler);
				}).catch(discordErrorHandler);

		} else { // fazendo lance
			if (!game)
				return msg.reply("não tem nenhum bingo acontecendo no momento").catch(discordErrorHandler);

			const table = game.players[msg.author.id];
			if (!table)
				return msg.reply("você não está nessa partida de bingo").catch(discordErrorHandler);
			...
			if (checkWin(game.played, table)) {
				...
				msg.reply("tem certeza? acho que não").catch(discordErrorHandler);
			}
		}
	},
	aliases: ["bingo"],
