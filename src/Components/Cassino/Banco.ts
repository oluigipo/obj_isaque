...
export interface usersJsonUser {
    userid: string;
    money: number;
    messages: number;
    last: number;
...
const teste: usersJson = { users: [] };
export const Banco = {
    json: teste,
    jsonOpened: false,
    jsonOpen() {
        const raw: string = fs.readFileSync(userData, { encoding: 'utf8' });
    ...
    userinfo(userid: string): usersJsonUser | undefined {
        this.jsonOpen();
    ...
    upgradeMedalha(userid: string): number {
        return 0;
    }
}